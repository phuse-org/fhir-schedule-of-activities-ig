...
2. **Resolve encounter timing.** For each `Encounter`, determine whether it is a
   timeline anchor or is timed relative to another encounter, and convert its `Timing`
...
This is the most structurally different part of the two models and the part most
likely to need careful porting into other stacks.
...
- **If `encounter.scheduledAtId` is null**, the encounter is a timeline **anchor**.
  Emit `soaTimePointType`/`soaTimePointSubType` only — no `soaPlannedTimePoint`, no
  `soaPlannedRange`, and no `relatedAction`. Note that an encounter can be an anchor
  even if it has a `previousId`: USDM allows a treatment anchor (e.g. Baseline)
  partway through the encounter chain, distinct from the very first encounter. Anchor
  status is driven solely by `scheduledAtId == null`, not by chain position.
- **Otherwise**, resolve `encounter.scheduledAtId` to its `Timing`, and convert
  `Timing.value` and any `windowLower`/`windowUpper` from ISO 8601 duration to days
  (§3.3). The encounter this timing is measured *from* is named by
  `encounter.previousId` — not by `Timing.relativeFromScheduledInstanceId`, which
  identifies the scheduled activity instance that *uses* the timing, not the encounter
  the timing is measured from. Emit the full `soaTimepoint` extension, plus a
  `relatedAction` referencing the resolved reference encounter with
  `relationship = #after` and the resolved window as `offsetRange`.
...
### 3.4 Timepoint subtype derivation

USDM carries no dedicated field for visit-role subtype; it must be derived, typically
from the encounter's label or a similar descriptive field. A representative mapping:
...
3. **Encounter order** for both the visit list and the master schedule
   ...
   file, both of which can be non-sequential.
...
- [USDM Transform](usdm.html) — how to run a reference pipeline against this IG's
  example study
